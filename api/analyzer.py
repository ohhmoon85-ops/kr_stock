...
단기 매수 추천 종목 5개를 포함하세요. 장기 종목은 제외합니다.

[중요 규칙]
- 진입가/목표가/손절가는 반드시 위 데이터에 있는 실제 종가(close)를 기반으로 계산하세요.
- 데이터에 없는 종목의 가격을 추정하거나 임의로 생성하지 마세요.
- 한국 종목 데이터가 제공된 경우, 해당 종목의 실제 종가만 사용하세요.
- 미 증시 데이터가 없을 경우 "데이터 수집 중 오류"로 표기하고 가용 데이터로만 분석하세요."""
